...
temp_blackcards = cards[:blackCards].map.with_index{|card, i| card.merge({id: i+1})}
temp_whitecards = cards[:whiteCards].map.with_index{|card, i| {}.merge({"id": i+1,"text": card})}

RENDER_CARDS = {
  "blackCards": temp_blackcards,
  "whiteCards": temp_whitecards
}
